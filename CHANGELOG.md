...
- **README updates** (`README.md`) -- Reconnaissance Pipeline intro paragraph updated to describe the 3-way parallel vulnerability phase (Nuclei ∥ GraphQL ∥ Subdomain Takeover). Recon Pipeline Tool Matrix gains a new **Subdomain Takeover** row and the existing Nuclei + GraphQL rows updated to reflect three parallel siblings in Phase A (was two). New **Subdomain Takeover Detection** feature-highlight section mirrors the GraphQL entry: engine stack, scoring rules, 40+ supported providers, 12-provider auto-exploitable bonus list, graph output, and stealth-mode behavior. Dev-mode "When to Rebuild vs Restart" table already documents `baddns_scan/Dockerfile` -> `docker compose --profile tools build baddns-scanner`
